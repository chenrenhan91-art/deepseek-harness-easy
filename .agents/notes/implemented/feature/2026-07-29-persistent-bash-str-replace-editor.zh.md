...
随附的 [`minimal` JSON-RPC 组合](../../../../examples/jsonrpc-agent/minimal.cordis.yml) 会组合这两个插件，以满足与 Claude SWE 兼容的 RL 约定。其 entry 本地 PTY realm 持有注册表、本地后端和持久 Bash 工具；编辑器在该 realm 旁注册，并使用本地文件系统。这一组合边界由 [minimal-preset 决策](../bug-fix/2026-08-10-minimal-preset-owns-rl-composition.md)负责说明。Web 新手模式不挂载这两个插件（[新手 Web 工作台](./2026-08-15-beginner-web-workbench.md)）。
