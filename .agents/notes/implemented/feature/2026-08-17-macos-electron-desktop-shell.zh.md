...
**在用户这台 Mac 上，克隆并构建之后，`pnpm run desktop:install` 写入 `~/Desktop/DeepSeek Harness.app`，并打开 Electron 窗口，加载本检出的 `pnpm dsh web`（`http://127.0.0.1:3080`）。** `.app` 在本机生成，不会像下载来的 Release zip 那样被加上互联网隔离。启动脚本会写入克隆、Node、pnpm 和 Electron 的绝对路径，因为访达启动时 PATH 很短。
...
- `apps/desktop/tests/desktop.spec.ts` 钉住 spawn argv、把 Info.plist/启动脚本装进临时目录、端口探测，以及写入路径的引号。
