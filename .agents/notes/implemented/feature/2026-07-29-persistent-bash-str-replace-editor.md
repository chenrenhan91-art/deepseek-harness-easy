...
The shipped [`minimal` JSON-RPC composition](../../../../examples/jsonrpc-agent/minimal.cordis.yml) composes both plugins for the Claude SWE-compatible RL contract. Its entry-local PTY realm carries the registry, local backend, and persistent Bash tool; the editor registers beside that realm against the local filesystem. The [minimal-preset decision](../bug-fix/2026-08-10-minimal-preset-owns-rl-composition.md) owns this composition boundary. Web beginner modes do not mount these plugins ([beginner Web workbench](./2026-08-15-beginner-web-workbench.md)).
