...
**每个权限选择器都把 `danger-full-access` 关进共享的页面内 `RiskConfirmation` 对话框：启用按钮在用户勾选明确的风险确认复选框前保持禁用；选择器原样渲染宿主 `name`；所有取消路径都不作任何提交。**
