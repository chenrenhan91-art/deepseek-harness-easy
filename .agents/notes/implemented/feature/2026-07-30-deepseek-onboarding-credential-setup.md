...
**The credential step is a full-page guide.** After the notice completes, a missing writable DeepSeek key renders `ApiKeyOnboarding` over the whole viewport: where to create a key, one field, a probe via `llm.discoverModels`, then `credentials.set`. The [beginner Web workbench](./2026-08-15-beginner-web-workbench.md) owns that page. Configure later completes only this coordinator pass. An absent adapter remains skipped because the browser cannot mount a missing Cordis plugin.
