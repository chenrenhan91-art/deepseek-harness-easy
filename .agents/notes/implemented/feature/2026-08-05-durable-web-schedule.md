...
The shipped Web composition loads `@deepseek-ai/dsh-time-context` and `@deepseek-ai/dsh-schedule`; [`examples/web-schedule`](../../../../examples/web-schedule/README.md) restates those Host rows so `dsh web --patch examples/web-schedule/cordis.yml` remains valid. Headless default compositions still omit both plugins. Schedule observes only root Agents published after the plugin loads and installs its three tools plus one disposable owner in that Agent scope. Cold history reads, already-published roots, child Agents, and other hosts do not activate it.

The user-visible boundary is `session-local`: the original Session runs an on-time reminder only while live, does no external notification while cold, and processes an overdue reminder after it becomes live again. Due work waits until the Agent is fully idle, then enters the ordinary next-turn queue through `followup()`; it never steers the current turn and has no independent Web receipt ([conversational delivery](../simplification/2026-08-09-conversational-schedule-delivery.md)). The shipped Web sidebar mounts a beginner schedule page over `/schedule` and the `schedule` projection ([beginner Web schedule surface](./2026-08-15-web-beginner-schedule-surface.md), [calendar daily and weekly Schedule](./2026-08-15-calendar-schedule-page.md)).
...
| Several recurring records are overdue | Each active record retains its earliest unaccepted target | One decision selects each record's latest occurrence and advances it past now | One ordinary follow-up containing one occurrence per record |
...
Calendar Cron expressions remain absent ([bounded recurrence simplification](../simplification/2026-08-09-bounded-fixed-rate-schedule.md)). Local-clock daily and weekly kinds are a separate durable rule set ([calendar daily and weekly Schedule](./2026-08-15-calendar-schedule-page.md)); they do not restore a Cron grammar or evaluator.

### Live delivery lifecycle

The Agent-scoped owner derives its earliest target from the durable fold. Long targets use bounded timer segments, and every wake reads the wall clock again, so a rollback cannot fire early and a forward jump becomes overdue. Due one-shots have priority and are admitted one at a time; otherwise every overdue Every, daily, and weekly record enters one batch in target and creation order. If a turn or maintenance task owns the Agent, `runMaintenance()` rejects the claim; the records stay active and one `whenIdle()` wait triggers another attempt. A rejected preflight or contained framing/enqueue failure also leaves them active without starting a private retry timer.
...
- Fixed-rate recurrence is bounded by a five-minute minimum, latest-only catch-up, and one batched occurrence per overdue record. Local-clock daily and weekly kinds are owned by [calendar daily and weekly Schedule](./2026-08-15-calendar-schedule-page.md); Cron remains absent.
