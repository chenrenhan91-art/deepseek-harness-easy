...
**Reject the source ZIP as a beginner download.** On macOS the supported beginner path is an agent following [INSTALL.md](../../../../INSTALL.md): clone this fork and run `pnpm run desktop:install` ([macos electron desktop shell](2026-08-17-macos-electron-desktop-shell.md)). Windows may still unzip `DeepSeek-Harness-Windows.zip` and double-click `DeepSeek Harness.exe`. That split is documented at the top of the root README.
...
**The launcher opens the system browser.** `dsh web` only prints the URL. The pack launcher starts bundled `node …/lib/bin.js web` when `127.0.0.1:3080` is not listening, then `open` / `start` that URL. Gatekeeper still quarantines an unsigned downloaded `.command`; that is not the macOS beginner path. The pack does not embed Electron or Tauri, and it does not productize a LaunchAgent or a `/Users/…` path. The clone-local Desktop `.app` is owned by [macos electron desktop shell](2026-08-17-macos-electron-desktop-shell.md).
...
**Electron or Tauri window around the same Web UI.** Rejected for the Release pack: the zip product is “open local Web in the system browser.” A second shell in the downloaded zip would add a signed-app toolchain without changing the bind or the onboarding page.
...
**Unsigned Finder launch as the macOS beginner path.** Rejected: Gatekeeper quarantines a downloaded `.command` and the dialog often has no Open button. The clone-and-`pnpm run desktop:install` path for that audience lives on [AI-assisted install](2026-08-16-ai-assisted-source-install.md).
