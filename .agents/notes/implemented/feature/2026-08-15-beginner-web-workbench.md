...
**Eight modes replace the four shipped presets.** `apps/cli/config/agent-presets/` holds `web-page`, `writing`, `sheet`, `files`, `study`, `slides`, `autopilot`, and `learn-code`. Each directory has Chinese `preset.yml` metadata, the same agent-plane tool roster (filesystem, shell, skill, plan, todo, web, schedule, ask-user, image read), a persona, and three original Chinese domain skills. `study` is the deployment default because it answers a question without producing files. All eight mount the shared vision skill at [`apps/cli/config/skills/vision/`](../../../../apps/cli/config/skills/vision/) via `../../skills/` in addition to their own `skills/`, and set `includeDefaultRoots: false` so `+` does not list `~/.agents/skills` or the workspace. `standard`, `code`, `minimal`, and `cordis` are absent from this roster. The JSON-RPC [`minimal.cordis.yml`](../../../../examples/jsonrpc-agent/minimal.cordis.yml) remains the two-tool RL composition for that launch path ([minimal-preset composition](../bug-fix/2026-08-10-minimal-preset-owns-rl-composition.md), [bare two-tool runtime](./2026-08-11-minimal-profiles-bare-two-tool-runtime.md)). Picking a mode arms its domain skills as composer tags ([mode skill pins](./2026-08-16-mode-skill-pins-and-preset-local-catalog.md)).
...
- A session or settings default that still names a deleted shipped id (`standard` / `code` / `minimal` / `cordis`) remaps through the unnamed default rather than refusing to open ([retired-preset resume](../bug-fix/2026-08-16-retired-preset-resume-remaps-to-default.md)).
