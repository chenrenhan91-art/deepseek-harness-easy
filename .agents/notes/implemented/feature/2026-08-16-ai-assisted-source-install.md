...
**Only write `npx @deepseek-ai/dsh web`.** Rejected as the written beginner procedure: it still needs Node, and this fork’s workbench lives in the git tree (`pnpm run desktop:install` after build). `npx` stays in the README developer section.
