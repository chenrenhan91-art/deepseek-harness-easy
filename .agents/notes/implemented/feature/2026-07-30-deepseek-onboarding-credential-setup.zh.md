...
**凭据步骤是整页引导。** 适配器已挂载且处于活跃状态，其引用可解析、可写但尚未配置时，`ApiKeyOnboarding` 会铺满视口：到哪里创建密钥、一个输入框、经 `llm.discoverModels` 探测，再调用 `credentials.set({ref, value})`。该页由[新手 Web 工作台](./2026-08-15-beginner-web-workbench.md)拥有。「稍后配置」只完成协调器当前这一轮。适配器缺失时仍跳过，因为浏览器不能挂载缺失的 Cordis 插件。
...
有序流程从产品声明页开始，无需重启即可进入整页密钥引导：无密钥浏览器测试在隔离的 harness 家目录下启动真实 Web 组合，确认声明后从该页把生成的密钥存入该目录的 `.credentials.yaml`，验证密钥未进入 DOM、ARIA 或浏览器控制台输出，并确认普通 Models 页面报告已配置。完整的无密钥 Web 回放也固定了同 id 的不可配置回放路由不会阻塞无关流程。纯就绪状态测试与 React 测试固化了受管文件凭据与进程环境凭据、提供方与能力缺失、取消、外部失效和协调器移交。该流程直接继承配置平面已记录的基础限制，不会另加局部的机密存储、脱敏或设置替换变通方案。
