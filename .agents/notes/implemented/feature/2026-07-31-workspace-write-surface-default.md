...
A genuinely fresh session on the shared base pins `permission/preset: workspace-write`, `sandbox/mode: workspace-write`, and `approval/policy: ask` before execution. The Web overlay then sets `permission.defaultPreset: danger-full-access`, so a fresh Web session pins Full access instead ([beginner Web workbench](./2026-08-15-beginner-web-workbench.md)). Existing and resumed sessions retain their logged permission, and changing the General-settings default affects only sessions created afterward. The browser keeps its Access picker, answerable approval cards, and risk confirmation for Full access. The TUI gains the existing `/permission` command because the shared Permission service activates its command child there.
...
The keyless shipped-TUI pseudo-terminal smoke, when present, boots the real Loader tree, reads the persisted first request, and asserts both the `sandbox_permissions`/`justification` bash schema and the initial workspace-write event triplet. The shipped-Web composition smoke asserts process `sandboxPolicy.defaultMode === 'workspace-write'` and `approval.config.policy === 'ask'`, and that `permissionPresets.defaultPreset` is `danger-full-access`. Assembled browser Settings snapshots open on 完全放开, preserve an existing session's pinned preset while changing the future default, and still prove the confirmed Full-access path after leaving that default.
...
Fresh sessions on the shared base can modify the active workspace and temporary roots without extra prompts, while an attempted mutation elsewhere is denied before it reaches the target. Web then pins Full access for new sessions ([beginner Web workbench](./2026-08-15-beginner-web-workbench.md)); leaving a safer preset still requires the acknowledgement dialog. Stored user defaults and logged session permissions are not rewritten.
