...
- 副本是完整快照，会随随附来源升级而漂移——接受；preset 层没有 patch 语义（那是 bundle 层 `cordis.patch.yml` 的能力），随附新手模式也为「一个文件读完整份组装」付了同样的代价：它们共用一套工具清单，只在人设和 `skills/` 上不同（[新手 Web 工作台](../feature/2026-08-15-beginner-web-workbench.md)）。
