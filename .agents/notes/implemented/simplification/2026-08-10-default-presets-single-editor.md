...
General-purpose agents use `read`, `write`, and `edit` for filesystem mutations, while the JSON-RPC minimal example retains `str_replace_editor`. Preset composition tests pin its absence from the beginner-mode roster and the Code Mode SDK, while the JSON-RPC assertions continue to pin its presence.
