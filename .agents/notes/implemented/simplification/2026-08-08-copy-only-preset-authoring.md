...
- A copy is a full snapshot that drifts from an upgraded shipped source — accepted; the preset layer has no patch semantics (that is the bundle layer's `cordis.patch.yml`), and the shipped beginner modes pay the same cost: they share one tool roster and differ by persona plus `skills/` ([beginner Web workbench](../feature/2026-08-15-beginner-web-workbench.md)).
