...
源码用户通过仓库脚本运行程序，而非使用已安装的 `dsh` 命令。仓库不提供原子升级切换，也不保留 staging 回滚检出；仓库同样不会自动集成个人源码修改或将其发布到上游。未来的分发机制必须说明为何应由其管理安装和升级状态，定义恢复行为，并补充测试与用户文档，同时不得让源码运行路径依赖该机制。桌面 Release 产品包就是这条后续路径：它发布已构建的 Web 切片和浏览器启动器，并不改变 `pnpm dsh web`（[桌面 Release 包](../feature/2026-08-16-desktop-release-pack.md)）。未来任何发布工作流都必须隔离出一项获批功能，并在首次推送和创建草稿 PR（Pull Request）前取得明确批准。
