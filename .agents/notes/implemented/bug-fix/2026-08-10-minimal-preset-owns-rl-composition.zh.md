...
JSON-RPC 的 [`minimal.cordis.yml`](../../../../examples/jsonrpc-agent/minimal.cordis.yml) 是随附的唯一双工具 RL 组合。Web 不再挂载 `minimal` preset；新手模式拥有 Web 的 agent 平面（[新手 Web 工作台](../feature/2026-08-15-beginner-web-workbench.md)）。该独立文件声明 entry 本地的 PTY 注册表与本地后端、带 RL 环境描述且超时为 300 秒的持久 `bash`，以及 `str_replace_editor`。工具呈现仍由部署选择。后续的[裸双工具运行时决策](../feature/2026-08-11-minimal-profiles-bare-two-tool-runtime.md)取代了本记录最初的压缩与文件系统提供方选择：当前示例挂载 `fs-local`，不挂载压缩后端。编辑器不接受 `requireAbsolutePath` 设置，因为要求绝对路径是它的无条件约定。
...
进程级 `core-web.cordis.yml` patch 不再存在。浏览器 UI、workspace 附加、持久化、子进程、沙箱、权限、模型路由及其他跨会话服务在 Web 上仍由宿主持有。JSON-RPC 示例是完整进程组合，不是 Web 会话 preset。

## 验证

系统提示词与 persona 包测试证明了 complete 段最终约束与 runtime-context 抑制，包括 waterfall 修改与重复项拒绝。独立的 [`minimal.cordis.yml`](../../../../examples/jsonrpc-agent/minimal.cordis.yml) 是内置 JSON-RPC 运行时的完整双工具组合。[裸双工具运行时决策](../feature/2026-08-11-minimal-profiles-bare-two-tool-runtime.md)说明其启动方式专属的环境配置、裸文件系统和无压缩选择。其无密钥 SDK 回放会断言组装后的系统提示词与双工具目录，跨调用执行持久 Bash，并使用编辑器；Python SDK 教程提供可运行的入口。
...
JSON-RPC 提示词由部署选择。模型只看到持久 `bash` 与 `str_replace_editor`；shell 状态按 agent 隔离，并随该 agent 一并消失。持久 shell 的本地后端需要受支持的 POSIX 终端基础环境，因此该组合不支持 Windows agent。Web 新手模式不挂载这套双工具约定（[新手 Web 工作台](../feature/2026-08-15-beginner-web-workbench.md)）。
