...
Web 不再随附第二个会话视图，因此双标签几何场景随 Trajectory 包一并退役（[新手 Web 工作台](../feature/2026-08-15-beginner-web-workbench.md)）。Chat 仍在列滚动容器上声明 `scrollbar-gutter: stable`；任何选用编辑器 overlay 的视图仍使用 CSS 补偿。单元测试可以断言这些声明存在，但无法证明两个标签页落在同一矩形上，因为只组合了 Chat。
