...
describe('web e2e: Full access confirmation', () => {
  ...
  it('requires acknowledgement before the composer picker can enable Full access', async () => {
    ...
    expect(await access.getAttribute('aria-label')).toBe('访问模式，当前：完全放开')
    ...
    const dialog = page.getByRole('dialog', { name: '确认启用完全放开？' })
    await dialog.waitFor({ timeout: 10_000 })
    const enable = dialog.getByRole('button', { name: '启用完全放开' })
