...
A compatibility overlay that restates the shipped Web `time-context` and `schedule` rows so `dsh web --patch examples/web-schedule/cordis.yml` remains valid. See [web-schedule/README.md](web-schedule/README.md) for absolute-time authority, delivery, and recovery boundaries.
