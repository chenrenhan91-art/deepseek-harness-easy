...
模型设置与产品引导插件。同一个 client Cordis 插件会注册 Models 页面，以及由 `settings.onboarding` 排序的两个首次使用步骤：本包弹窗中的版本化内测声明，随后是按条件显示、占满整屏的 API Key 引导页。Models 平面把三个协议领域汇聚为一个共享快照：`llm.providers`（可配置提供方目录，含每条路由的存活／休眠状态）、`settings.describe`（序列化 schema、分层脱敏值、secret slot）与 `credentials.describe`（不含值的 configured/source/writable 徽标）；页面据此渲染提供方行，一次只展开一张编辑卡片，且不把路由存活状态呈现为提供方状态。
...
声明完成后，DeepSeek 步骤会从同一个 Models 联接快照得出首次运行就绪状态。只要用户已经能触达**任何**一个提供方，它就直接完成而不渲染——已注册且其具名凭据引用已存储的路由（包括来自启动环境且只读的凭据），或 profile 根本不指名引用、因而走原生认证的路由。只有二者皆无的用户才会被要求填写 DeepSeek 官方密钥。适配器已挂载且活跃、引用可写但尚未配置时，引导页占满整个视口：官方「创建 Key」和「充值」链接、粘贴 Key 的那一个输入框，以及这把 Key 生效后助手将被允许做什么。密钥**先验证再存储**——`llm.discoverModels` 用这把 Key 向该路由自己的端点索取模型清单，因此填错会当场在输入框被拒，而不是等到第一句话失败；回复中的 `reason` 决定页面归咎于密钥还是网络。只有通过验证的密钥才会走到 `credentials.set`，它仍是唯一的 secret 写入，且不会改变提供方设置。「稍后再说」只完成协调器当前这一轮。适配器缺失、路由不活跃、联接失败、部署只读或设置／凭据能力不可用时，该步骤不渲染并直接完成；Models 页仍是诊断界面。
