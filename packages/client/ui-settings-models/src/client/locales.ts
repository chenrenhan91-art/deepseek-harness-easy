...
export const en = {
  ...
  onboardingTitle: 'Connect a model first',
  onboardingLead: 'One-time setup: DeepSeek runs the assistant, and an API key is how it charges you for that.',
  onboardingGetTitle: 'Step 1 — get a key',
  onboardingGetBody: 'Sign in to the DeepSeek platform, create an API key, and copy the string it shows once. API calls are prepaid: a key with no balance cannot run a turn, so open Top up when the account needs credit.',
  onboardingOpenConsole: 'Create an API key',
  onboardingOpenTopUp: 'Top up (official)',
  onboardingPasteTitle: 'Step 2 — paste and check',
  onboardingPasteBody: 'We ask DeepSeek for its model list with this key, so a mistyped key is caught here rather than in your first message.',
  onboardingVerify: 'Check and start',
  onboardingVerifying: 'Checking…',
  onboardingSaving: 'Saving…',
  onboardingKeyRefused: 'DeepSeek did not accept this key. Copy the whole key from the console and paste it again.',
  onboardingUnreachable: 'Could not reach DeepSeek. Check the network and try again.',
  onboardingSaveFailed: 'The key works, but saving it on this machine failed. Try again.',
  onboardingPermissionTitle: 'Before you start',
  onboardingPermissionBody: 'The assistant reads and writes files on this computer, runs commands, and browses the web without asking each time. Use it on your own machine, and back up anything you cannot lose.',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  onboardingTitle: '先连上模型，再开始',
  onboardingLead: '只需设置一次：助手由 DeepSeek 提供思考能力，API Key 就是它向你计费的凭据。',
  onboardingGetTitle: '第一步 · 领一个 Key',
  onboardingGetBody: '登录开放平台 → 创建 API Key → 需要付费时点「去充值」。API 按量扣费，没余额 Key 也跑不动。Key 只显示一次，请立刻复制。',
  onboardingOpenConsole: '去创建 API Key',
  onboardingOpenTopUp: '去充值（官方）',
  onboardingPasteTitle: '第二步 · 粘贴并验证',
  onboardingPasteBody: '我们会用这串 Key 向 DeepSeek 要一次模型清单，填错会当场提示，而不是等到你发第一句话。',
  onboardingVerify: '验证并开始',
  onboardingVerifying: '验证中…',
  onboardingSaving: '保存中…',
  onboardingKeyRefused: 'DeepSeek 不认这串 Key。请回控制台把完整的 Key 复制一遍，再粘贴进来。',
  onboardingUnreachable: '连不上 DeepSeek。请检查网络后重试。',
  onboardingSaveFailed: 'Key 可以用，但没能保存到这台电脑，请重试。',
  onboardingPermissionTitle: '开始前请知悉',
  onboardingPermissionBody: '助手会直接读写这台电脑上的文件、执行命令、访问网络，过程中不再逐条询问。请只在自己的电脑上使用，重要资料先备份。',
