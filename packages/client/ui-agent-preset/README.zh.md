...
agent preset 的各个表层：铺满新建会话界面的模式网格；General 设置中的一行，用于选择此后的会话据以组装的 [preset](../../preset/agent-presets/README.md)；会话标题旁的一个只读标签；以及当前模式领域 skill 的编辑器标签。
...
位于编辑器输入卡片正上方：当前会话模式自有 skill 的至多三枚标签。网格上的选择会刷新 `skill.list`，并把这些 `/name` 写进草稿，以便宿主在发送时注入 skill 正文。共享的 `vision` skill 留在 `+` 里，不会被钉上。点掉一枚标签会去掉对应 token；换模式会替换上一模式的 token，并留下用户正文。见[模式 skill 标签](../../../.agents/notes/implemented/feature/2026-08-16-mode-skill-pins-and-preset-local-catalog.md)。
