...
describe('command launcher chrome and control seats', () => {
  ...
  it('the Access chip renders the projection value and submits a non-Full-access pick directly', async () => {
    ...
    // The chip shows the host-supplied name; menu ids stay machine names.
    ...
    expect(items.map(o => o.textContent)).toEqual(['只读', '工作区可写', '完全放开'])
  ...
  it('requires explicit risk acknowledgement before submitting Full access', async () => {
    ...
    fireEvent.click(view.getByRole('menuitem', { name: '完全放开' }))

    expect(command).not.toHaveBeenCalled()
    expect(view.getByRole('dialog', { name: '确认启用完全放开？' })).toBeTruthy()
    const enable = view.getByRole('button', { name: '启用完全放开' }) as HTMLButtonElement
    ...
    expect((view.getByLabelText(/^访问模式/) as HTMLButtonElement).textContent).toBe('完全放开')
    await act(async () => {})
  })
  ...
  it('cancels a Full access selection without changing permission and resets acknowledgement', () => {
    ...
    expect((view.getByRole('button', { name: '启用完全放开' }) as HTMLButtonElement).disabled).toBe(true)
  })
