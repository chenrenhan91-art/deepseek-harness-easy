...
describe('WorkspaceRuntime', () => {
  ...
  it('connectWorkspace does not reuse a blank session that still names a retired shipped preset', async () => {
    const ctx = new Context()
    const api = new FakeApiClient()
    const sessions = new SessionRuntime(ctx, api, fakeRemote())
    const workspaces = new WorkspaceRuntime(ctx, api, sessions)
    api.onWorkspaceList = () => Promise.resolve(ok({
      items: [workspace('alpha', [sid('s-standard')])] as never[],
    }))
    api.onList = () => Promise.resolve(ok({
      items: [{
        sessionId: sid('s-standard'), updatedAt: 1, running: false, blank: true,
