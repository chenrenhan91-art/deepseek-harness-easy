...
describe('WorkspaceRuntime', () => {
  ...
  it.each(['standard', 'learn-code'] as const)(
    'connectWorkspace does not reuse a blank session that still names retired shipped preset %s',
    async (agentPreset) => {
      const ctx = new Context()
      const api = new FakeApiClient()
      const sessions = new SessionRuntime(ctx, api, fakeRemote())
      const workspaces = new WorkspaceRuntime(ctx, api, sessions)
      api.onWorkspaceList = () => Promise.resolve(ok({
        items: [workspace('alpha', [sid('s-retired')])] as never[],
      }))
      api.onList = () => Promise.resolve(ok({
        items: [{
          sessionId: sid('s-retired'), updatedAt: 1, running: false, blank: true,
          cwd: '/w/alpha', agentPreset,
        }] as never[],
      }))
      await Promise.all([workspaces.refresh(), sessions.refresh()])
      await Promise.resolve()
