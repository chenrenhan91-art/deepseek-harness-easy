...
菜单查询会按顺序且不区分大小写地模糊匹配命令名的子序列。前缀排名最高；其余匹配项按分隔符边界优先、相邻字符优先、间隔越短越优先的规则排序，若仍同分，则以目录顺序和贡献项顺序打破平局。此行为只影响命令发现：space 和 Enter 仍要求命令名精确匹配。原理：[Web 斜杠命令模糊发现](../../../.agents/notes/implemented/feature/2026-08-04-web-slash-command-fuzzy-discovery.md)。本包 `command` 词典中带有 `description.<name>` 键的 Host 目录行在菜单中使用该 locale 字符串；其他 Host 名称保持目录说明。原理：[本地化斜杠菜单说明](../../../.agents/notes/implemented/feature/2026-08-15-localized-slash-menu-and-vision-skill.md)。
