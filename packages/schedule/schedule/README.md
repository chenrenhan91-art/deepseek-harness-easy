...
`dsh-schedule` gives future live root Agents three Session-scoped tools for durable reminders. Version 1 accepts positive safe-integer `after_seconds` delays, explicit absolute `at` targets, fixed-rate `every_seconds` intervals of at least five minutes, and local-clock `daily` / `weekly` rules. The Session event log owns reminder state; timers, tool values, and model follow-ups are disposable projections of that log.
...
Time-context is not a Schedule dependency. A composition may mount `@deepseek-ai/dsh-time-context` so the model can interpret natural language in the browser's request-local zone; the shipped Web composition does. The model must still pass an explicit offset or `time_zone` to `schedule_create`; Schedule never imports or infers from model context.

When `ctx.commands` is composed, `/schedule` shares the create and delete transactions with the tools. When `ctx.sessionProjections` is composed, the `schedule` unit serves the active list to Web. Neither child is required for headless tool-only assemblies.
...
Schedule owns deterministic calendar normalization. Local times inside a daylight-saving gap are rejected for one-shot `at`. An overlap chooses its first, earlier instant. A successful `at` create retains only canonical UTC `scheduledAt`; no Schedule path reads the browser, Session header, model time-context, connection, or process time zone.

## Local-clock daily and weekly input

`daily` is `{ time, time_zone }` and `weekly` is `{ time, time_zone, weekdays }`. `time` is `HH:mm` or `HH:mm:ss`; stored records always use canonical `HH:mm:ss`. `time_zone` is explicit `UTC` or IANA Area/Location. Weekly weekdays are ISO integers, Monday = 1 through Sunday = 7, stored unique and sorted. A daylight-saving gap has no instant on that date, so create and dispatch walk to the next matching date where the clock exists; an overlap still chooses the earlier instant.

The first target is the next matching local clock strictly after create time. Recurring dispatch stores `acceptedAt` and advances with the same latest-only catch-up as Every, using calendar search rather than integer intervals. Cron expressions are not accepted.
...
When one or more recurring records are overdue, the package queues one stable user-role framing. `reminders_json` is a JSON array in target and creation order; each object has `schedule_id`, the selected latest `occurrence_at`, and the `reminder_prompt` supplied at creation:
...
Each admitted recurring batch adds one data-dependent user-role message regardless of how many distinct overdue records are due. It remains in Session history and contributes tokens until ordinary compaction removes or replaces that history.
...
- **Fixed intervals and local clocks, not Cron** — `every_seconds` is creation-anchor-aligned and cannot run more often than every five minutes; `daily` and `weekly` store a local clock in an explicit IANA zone. Cron expressions are not part of the protocol.
- **Latest-only catch-up** — an overdue recurring record contributes only its latest due occurrence, so Schedule never replays a missed backlog.
