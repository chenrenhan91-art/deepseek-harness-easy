...
### Reading a model listing (`model-listing.ts`)

Every adapter interrogating an OpenAI-compatible `GET /models` reads the same reply, so the two shared halves live here while each adapter keeps its own request — only it knows its endpoint, its protocol, and which credential to send. `modelListingUrl(baseURL)` appends `/models` as a prefix rather than resolving a URL, so a deployment path such as `https://gateway.example/openai/v1` keeps its segments. `readModelListing(response, url)` reads the body under a four-megabyte ceiling enforced on the bytes actually received (a declared length is checked first but never trusted), parses it, and maps each entry, skipping one without a usable id rather than failing the whole listing. An oversized body, a non-JSON body, and a body with no `data` array all fail with `DISCOVERY_FAILED` naming the endpoint.
