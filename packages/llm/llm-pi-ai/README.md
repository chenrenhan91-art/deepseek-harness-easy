...
The reply is read by `readModelListing` from [`@deepseek-ai/dsh-llm`](../llm/README.md#reading-a-model-listing-model-listingts), shared with every other adapter reading the same OpenAI-compatible listing: an id and whatever capacities the gateway discloses, entries without a usable id skipped, and a four-megabyte ceiling on the bytes actually received. An unreachable endpoint, a refused credential, a non-JSON body, and a body with no `data` array all fail with `DISCOVERY_FAILED` and a message naming the endpoint and, for a 401 or 403 alone, the credential. Cancellation during the body read surfaces as `ABORTED`, like a cancellation before the request went out.
