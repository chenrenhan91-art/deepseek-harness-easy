...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    keyDomain: 'fixed by the owner\'s key table { [Kind in ChatNodeKind]: { node: ChatNode<Kind> } }, already taken: assistant-step, command, compaction, context, manual-compaction, model-retry, steering, tool-call, turn-error, turn-max-tokens, turn-tail, unknown, user',
    hookContext: 'string',
  ...
  },
  {
    ...
    kind: 'single',
    scope: 'root',
    summary: 'The mode grid under the composer card on the new-session screen: the full-width seat where the user picks what kind of agent to talk to.',
    doc: 'The mode grid under the composer card on the new-session screen: the\nfull-width seat where the user picks what kind of agent to talk to.\nRoot scope: no session exists yet, so the choice is staged for the next\none rather than applied to a current one.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'conversation\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-agent-preset ModeGrid',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  },
  {
    ...
    kind: 'list',
    scope: 'session',
    summary: 'The conversation view ring: one list entry per view tab (chat here; further tabs from other plugins), rendered one-at-a-time by the session body via `only: <active id>`.',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-settings-models ApiKeyOnboarding id \'deepseek-official\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      'client-ui-schedule ScheduleSidebar id \'schedule\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'sidebar.session.action\', () => ctx.slots.register(\n      { name: \'sidebar.session.action\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    keyDomain: 'open: any string the owner dispatches (no compile-time key set), already taken: ask_user_question, bash, edit, glob, grep, read, skill, todo_write, web_fetch, web_search, write',
    hookContext: '',
    slotInject: '',
